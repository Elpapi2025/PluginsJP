...
public class WallListener extends Module<WallManager> {
    ...
    public void onMove(PlayerMoveEvent e) {
        ...
        if (Config.TEAM_CITADEL_ENTER_LIMIT > 0 && !player.hasPermission("azurite.citadel.entry") && to instanceof CitadelTeam) {
        ...
        if (Config.TEAM_CONQUEST_ENTER_LIMIT > 0 && !player.hasPermission("azurite.conquest.entry") && to instanceof ConquestTeam) {
            PlayerTeam pt = getInstance().getTeamManager().getByPlayer(player.getUniqueId());
        ...
        if (Config.TEAM_EVENT_ENTER_LIMIT > 0 && !player.hasPermission("azurite.event.entry") && to instanceof EventTeam) {
            PlayerTeam pt = getInstance().getTeamManager().getByPlayer(player.getUniqueId());
