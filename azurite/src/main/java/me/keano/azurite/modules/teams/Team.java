...
public class Team extends Module<TeamManager> {
    ...
    public Team(TeamManager manager, Map<String, Object> map, boolean deathban, TeamType type) {
        ...
        this.claims = Utils.createList(map.get("claims"), String.class)
                .stream()
                .map(Serializer::deserializeClaim)
        ...
        if (map.containsKey("hq")) this.hq = Serializer.deserializeLoc((String) map.get("hq"));
        ...
        if (isSystemTeam()) {
            getManager().getSystemTeams().put(uniqueID, this);
        }
    }
    ...
    public List<String> getTeamInfo(CommandSender sender) {
        return Arrays.asList(
                CC.LINE,
                (sender instanceof Player ? getDisplayName((Player) sender) : name) + " &3- &eHQ: &f" + getHQFormatted(),
    ...
    public Map<String, Object> serialize() {
        ...
        map.put("name", name);
        map.put("claims", claims.stream().map(Serializer::serializeClaim).collect(Collectors.toList()));
