...
import me.juanpiece.titan.modules.teams.menus.TeamBaseWallMenu;
 ...
 */
public class TeamBaseCommand extends Argument {

    private final Map<UUID, PlayerClaim> claimMap;
    private final Cooldown cooldown;
    private final ItemStack claimWand;

    public TeamBaseCommand(CommandManager manager) {
        super(
                manager,
        ...
        this.claimWand = new ItemBuilder(ItemUtils.getMat(manager.getTeamConfig().getString("BASE_CONFIG.BASE_WAND.TYPE")))
                .setName(manager.getTeamConfig().getString("BASE_CONFIG.BASE_WAND.NAME"))
                .setLore(manager.getTeamConfig().getStringList("BASE_CONFIG.BASE_WAND.LORE"))
                .toItemStack();
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (player.getInventory().firstEmpty() == -1) {
            sendMessage(sender, getLanguageConfig().getString("TEAM_COMMAND.TEAM_BASE.INVENTORY_FULL"));
            return;
        }
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        if (action == Action.RIGHT_CLICK_AIR) {
            ...
            sendMessage(player, getLanguageConfig().getString("TEAM_COMMAND.TEAM_BASE.CANCELLED_SELECTION"));
            return;
        }

        if ((e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK) && player.isSneaking()) {
            if (location1 == null || location2 == null) {
                sendMessage(player, getLanguageConfig().getString("TEAM_COMMAND.TEAM_BASE.NEVER_SELECT_ALL"));
                return;
            }

            Claim claimObject = new Claim(pt.getUniqueID(), location1, location2);
            claimObject.setY1(Math.min(location1.getBlockY(), location2.getBlockY()));
            claimObject.setY2(Math.max(location1.getBlockY(), location2.getBlockY()));
            getManager().setItemInHand(player, new ItemStack(Material.AIR));
            clearSelection(player);
            new TeamBaseWallMenu(getInstance().getMenuManager(), player, claimObject).open();
            return;
        }
        ...
        if (action == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (atBlock != pt) {
                sendMessage(player, getLanguageConfig().getString("TEAM_COMMAND.TEAM_BASE.CANNOT_CREATE_HERE"));
                return;
            }
