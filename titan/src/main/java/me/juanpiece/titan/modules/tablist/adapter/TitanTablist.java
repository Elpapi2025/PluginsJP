package me.juanpiece.titan.modules.tablist.adapter;
...
import me.juanpiece.titan.modules.tablist.TablistAdapter;
import me.juanpiece.titan.modules.tablist.TablistManager;
import me.juanpiece.titan.modules.tablist.extra.TablistData;
import me.juanpiece.titan.modules.tablist.extra.TablistPlaceholder;
 ...
 */
public class TitanTablist extends Module<TablistManager> implements TablistAdapter {
    ...
    private final Table<Integer, Integer, List<TablistPlaceholder>> placeholders;
    ...
    public TitanTablist(TablistManager manager) {
        ...
        this.placeholders = HashBasedTable.create();
    ...
    private void cachePlaceholders(int col, int row, String string) {
        ...
        if (string.contains("%team-")) {
            ...
            list.add((data, line, add) -> {
                ...
                line = line.replace("%team-" + number + "%", data.getTeamListString()
        ...
        if (string.contains("%scheduled%")) {
            list.add((data, line, add) -> {
                line = line.replace("%scheduled%", data.getSchedule().getKey());
                if (add) data.getInfo().add(col, row, line);
                return line;
            });
        }
        ...
        if (string.contains("%scheduledtime%")) {
            list.add((data, line, add) -> {
                line = line.replace("%scheduledtime%", Formatter.formatSchedule(data.getSchedule().getValue()));
                if (add) data.getInfo().add(col, row, line);
                return line;
            });
        }
    ...
    public String[] getHeader(Player player) {
        ...
        for (int i = 0; i < finalHeader.length; i++) {
            ...
            finalHeader[i] = getInstance().getPlaceholderHook().replace(player, string)
                    .replace("%online%", String.valueOf(Bukkit.getOnlinePlayers().size() - staff))
    ...
    public Tablist getInfo(Player player) {
        ...
        Pair<String, Long> schedule = getInstance().getScheduleManager().getNextSchedule();
        List<PlayerTeam> teamList = getInstance().getTeamManager().getTeamSorting().getList(player);

        String teamListString = (user.getTeamListSetting().name().contains("DTR") ?
                Config.TABLIST_LIST_FORMAT_DTR :
                Config.TABLIST_LIST_FORMAT_ONLINE
        );

        int staff = getInstance().getStaffManager().getVanished().size();
        TablistData data = new TablistData(info, user, pt, atPlayer, schedule, teamListString, player, teamList, staff);
