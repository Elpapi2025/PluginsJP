...
public class BardClass extends PvPClass {
    ...
    public void onHeld(PlayerItemHeldEvent e) {
        ...
        if (item != null && players.contains(player.getUniqueId())) {
            ...
            // This is a bug causing a player to be invincible, patched by delaying.
            if (effect.getEffect().getType().equals(PotionEffectType.REGENERATION)) {
                if (regenCooldown.hasCooldown(player)) return;
                regenCooldown.applyCooldownTicks(player, 3000); // 3s
            }
