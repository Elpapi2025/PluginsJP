...
import me.juanpiece.titan.modules.ability.type.FocusModeAbility;
...
import me.juanpiece.titan.modules.pvpclass.type.rogue.RogueBackstabEvent;
...
public class DeathListener extends Module<ListenerManager> {
    ...
    public void onDeath(PlayerDeathEvent e) {
        ...
        if (player.getLastDamageCause() instanceof RogueBackstabEvent) {
            // RogueBackstabEvent extends EntityDamageEvent which allows me to use it.
            RogueBackstabEvent rogueBackstabEvent = (RogueBackstabEvent) player.getLastDamageCause();
            String formatted = formatRogueEvent(player, rogueBackstabEvent);
            getInstance().getTeamManager().handleDeath(player, rogueBackstabEvent.getBackstabbedBy(), formatted);
            e.setDeathMessage(formatted.isEmpty() ? null : formatted);

            // Credits: https://www.spigotmc.org/threads/playerdeathevent.393112/
        } else if (player.getLastDamageCause() instanceof EntityDamageByEntityEvent) {
            String formatted = formatDamageEvent(player, (EntityDamageByEntityEvent) player.getLastDamageCause(), cause);
            Player killer = getKiller(player);
            getInstance().getTeamManager().handleDeath(player, getKiller(player), formatted);
            handleStats(killer, player);
            e.setDeathMessage(formatted.isEmpty() ? null : formatted);

        } else {
            String formatted = formatEvent(player, cause);
            Player killer = getKiller(player);
            getInstance().getTeamManager().handleDeath(player, killer, formatted);
            handleStats(killer, player);
            e.setDeathMessage(formatted.isEmpty() ? null : formatted);
        }
    }
