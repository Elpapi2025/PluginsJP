package me.juanpiece.titan.modules.storage.mongo;
...
public class MongoStorage extends Module<StorageManager> implements Storage {
        ...
        for (Document document : teamsCollection.find()) {
            ...
            if (toSave == null) {
                getInstance().getLogger().log(
                        Level.SEVERE,
                        "[Titan] Error occurred while loading a team! Report immediately! (MONGO)"
                );
                continue;
            }
    ...
    public void loadUsers() {
        for (Document document : usersCollection.find()) {
            new User(getInstance().getUserManager(), document); // that simple
        }
    }
