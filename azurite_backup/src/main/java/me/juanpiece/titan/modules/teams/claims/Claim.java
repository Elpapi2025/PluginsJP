package me.juanpiece.titan.modules.teams.claims;

import com.google.common.base.Preconditions;
import lombok.Getter;
import lombok.Setter;
import me.juanpiece.titan.utils.cuboid.CuboidBlockIterator;
import me.juanpiece.titan.utils.cuboid.CuboidDirection;
import me.juanpiece.titan.utils.cuboid.CuboidLocationIterator;
import org.bukkit.*;
...
@Setter
/*
Credits iHCF: Merged with claim object
 */
public class Claim implements Iterable<Block>, Cloneable {
    ...
    public Claim(UUID team, Location first, Location second) {
    ...
    public Claim(Claim claim) {
        Preconditions.checkNotNull((Object) claim.worldName, "World name cannot be null");
        this.worldName = claim.worldName;
        this.team = claim.team;
        this.x1 = Math.min(claim.x1, claim.x2);
        this.y1 = 0;
        this.z1 = Math.min(claim.z1, claim.z2);
        this.x2 = Math.max(claim.x1, claim.x2);
        this.y2 = 256;
        this.z2 = Math.max(claim.z1, claim.z2);
        this.locked = false;
    }
    ...
    public Set<Player> getPlayers() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (this.contains(player)) players.add(player);
        }
    ...
    public Block[] getCornerBlocks() {
        ...
        result[2] = world.getBlockAt(this.x2, this.y1, this.z1);
        result[3] = world.getBlockAt(this.x2, this.y1, this.z2);
        return result;
    }

    public Claim shift(CuboidDirection direction, int amount) throws IllegalArgumentException {
        return this.expand(direction, amount).expand(direction.opposite(), -amount);
    }

    public Claim inset(CuboidDirection direction, int amount) throws IllegalArgumentException {
        return this.outset(direction, -amount);
    }

    public List<Block> getWalls(int level, int height) {
        List<Block> blocks = new ArrayList<>();
        World world = getWorld();

        for (int x = this.x1; x <= this.x2; x++) {
            for (int i = level; i <= height; i++) {
                if (world.isChunkLoaded(x >> 4, z1 >> 4)) blocks.add(world.getBlockAt(x, i, this.z1));
                if (world.isChunkLoaded(x >> 4, z2 >> 4)) blocks.add(world.getBlockAt(x, i, this.z2));
            }
        }

        for (int y = level; y <= height; y++) {
            for (int z = this.z1; z <= this.z2; z++) {
                if (world.isChunkLoaded(x1 >> 4, z >> 4)) blocks.add(world.getBlockAt(this.x1, y, z));
                if (world.isChunkLoaded(x2 >> 4, z >> 4)) blocks.add(world.getBlockAt(this.x2, y, z));
            }
        }
    ...
    public Claim expand(CuboidDirection direction, int amount) throws IllegalArgumentException {
        switch (direction) {
            case NORTH: {
    ...
    public Claim outset(CuboidDirection direction, int amount) throws IllegalArgumentException {
        switch (direction) {
            case HORIZONTAL: {
    ...
    public boolean contains(Claim cuboid) {
    ...
    public Claim contract(CuboidDirection direction) {
        Claim face = this.getFace(direction.opposite());
        switch (direction) {
            case DOWN: {
                while (face.containsOnly(Material.AIR) && face.y1 > this.y1) {
                    face = face.shift(CuboidDirection.DOWN, 1);
                }
                return new Claim(this.worldName, this.x1, this.y1, this.z1, this.x2, face.y2, this.z2);
            }
            case UP: {
                while (face.containsOnly(Material.AIR) && face.y2 < this.y2) {
                    face = face.shift(CuboidDirection.UP, 1);
                }
                return new Claim(this.worldName, this.x1, face.y1, this.z1, this.x2, this.y2, this.z2);
            }
            case NORTH: {
                while (face.containsOnly(Material.AIR) && face.x1 > this.x1) {
                    face = face.shift(CuboidDirection.NORTH, 1);
                }
                return new Claim(this.worldName, this.x1, this.y1, this.z1, face.x2, this.y2, this.z2);
            }
            case SOUTH: {
                while (face.containsOnly(Material.AIR) && face.x2 < this.x2) {
                    face = face.shift(CuboidDirection.SOUTH, 1);
                }
                return new Claim(this.worldName, face.x1, this.y1, this.z1, this.x2, this.y2, this.z2);
            }
            case EAST: {
                while (face.containsOnly(Material.AIR) && face.z1 > this.z1) {
                    face = face.shift(CuboidDirection.EAST, 1);
                }
                return new Claim(this.worldName, this.x1, this.y1, this.z1, this.x2, this.y2, face.z2);
            }
            case WEST: {
                while (face.containsOnly(Material.AIR) && face.z2 < this.z2) {
                    face = face.shift(CuboidDirection.WEST, 1);
                }
                return new Claim(this.worldName, this.x1, this.y1, face.z1, this.x2, this.y2, this.z2);
            }
            default: {
                throw new IllegalArgumentException("Invalid direction " + direction);
            }
