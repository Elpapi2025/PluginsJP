...
public class LimiterListener extends Module<ListenerManager> {
    ...
    private void load() {
        ...
        for (PotionEffectType effect : PotionEffectType.values()) {
            ...
            boolean enabled = getLimitersConfig().getBoolean("POTIONS." + effect.getName() + ".ENABLED");
            boolean upgradable = getLimitersConfig().getBoolean("POTIONS." + effect.getName() + ".UPGRADABLE");
            boolean extended = getLimitersConfig().getBoolean("POTIONS." + effect.getName() + ".EXTENDED");
    ...
    public void onEffect(PotionSplashEvent e) {
        ...
        if (!player.hasPermission("titan.potionlimit.bypass") && cancelPotion(entity.getItem())) {
            e.setCancelled(true);
    ...
    public void onDrink(PlayerItemConsumeEvent e) {
        ...
        if (!player.hasPermission("titan.potionlimit.bypass") && cancelPotion(item)) {
            e.setCancelled(true);
            player.sendMessage(getLanguageConfig().getString("LIMITER_LISTENER.DENIED_POTION"));
        }
    }
