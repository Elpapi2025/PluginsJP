package me.juanpiece.titan.modules.waypoints.listener;
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        if (conquest.isActive()) {
            for (Capzone capzone : conquest.getCapzones().values()) {
                getManager().getConquestWaypoint().send(player, capzone.getZone().getCenter(), s -> s.replace("%name%", capzone.getType().getName()).replace("%color%", String.valueOf(Utils.translateChatColorToColor(capzone.getType().getColor()).getRGB())));
            }
        }
