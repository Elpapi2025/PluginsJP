...
public class TeamSetCapsArg extends Argument {
    public TeamSetCapsArg(CommandManager manager) {
        super(manager, Arrays.asList("setkothcaptures", "setcaptures"));
        this.setPermissible("titan.team.setcaps");
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        team.setKothCaptures(amount);
        team.save();
        this.sendMessage(sender, this.getLanguageConfig().getString("ADMIN_TEAM_COMMAND.TEAM_SETCAPTURES.SETCAPTURES").replaceAll("%team%", args[0]).replaceAll("%amount%", args[1]));
    }
    
    @Override
    public String usage() {
        return this.getLanguageConfig().getString("ADMIN_TEAM_COMMAND.TEAM_SETCAPTURES.USGE");
    }
}
