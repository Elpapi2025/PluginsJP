...
public class InvincibilityTimer extends PlayerTimer {
    ...
    public void onPortal(PlayerPortalEvent event) {
        ...
        if (this.hasTimer(player) && !this.getConfig().getBoolean("INVINCIBILITY.END_ENTRY")) {
            event.setCancelled(true);
            if (this.portalCooldown.hasCooldown(player)) {
                return;
            }
            player.sendMessage(this.getLanguageConfig().getString("INVINCIBILITY.DENIED_END_ENTRY"));
    ...
    public void onTeleport(PlayerTeleportEvent event) {
        ...
        if (this.hasTimer(player)) {
            if (playerTeam != null && playerTeam.getUniqueID().equals(team.getUniqueID()) && this.getConfig().getBoolean("INVINCIBILITY.ENTER_OWN_CLAIM")) {
                return;
            }
            if (team instanceof PlayerTeam || team instanceof EventTeam) {
                ...
                player.sendMessage(this.getLanguageConfig().getString("INVINCIBILITY.CANNOT_TELEPORT").replaceAll("%claim%", team.getDisplayName(player)));
            }
        }
    }
    ...
    public boolean checkEntry(Player player, Team team) {
        ...
        if (team instanceof PlayerTeam) {
            ...
            boolean ownClaim = this.manager.getConfig().getBoolean("INVINCIBILITY.ENTER_OWN_CLAIM");
            if (ownClaim && playerTeam.getPlayers().contains(player.getUniqueId())) {
                return false;
            }
        }
    ...
    public InvincibilityTimer(TimerManager manager) {
        super(manager, true, "Invincibility", "PLAYER_TIMERS.INVINCIBILITY", manager.getConfig().getInt("TIMERS_COOLDOWN.INVINCIBILITY"));
