...
public class GMSCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length != 1) {
            Logger.info(player.getName() + " has switched gamemodes to survival. (/gms)");
        ...
        Logger.info(player.getName() + " has switched the gamemode of " + target.getName() + "  to survival. (/gms)");
    ...
    public List<String> usage() {
        return this.getLanguageConfig().getStringList("GMS_COMMAND.USAGE");
    }
    
    public GMSCommand(CommandManager manager) {
        super(manager, "gms");
        this.setPermissible("titan.gms");
    }
