...
public class TeamListener extends HCFModule<TeamManager> {
    ...
    public void onBreak(BlockBreakEvent event) {
        ...
        if (team instanceof MountainTeam) {
            ...
            if (mountainTeam.getAllowedBreak().contains(event.getBlock().getType())) {
                return;
            }
        }
        if (!this.getManager().canBuild(player, event.getBlock().getLocation())) {
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (team instanceof MountainTeam) {
            MountainTeam mountainTeam = (MountainTeam)team;
            if (mountainTeam.getChests().contains(block.getLocation())) {
                return;
            }
        }
        if (team instanceof CitadelTeam) {
